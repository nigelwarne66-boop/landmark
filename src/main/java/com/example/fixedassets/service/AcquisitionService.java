...
import com.example.fixedassets.model.AcquisitionRecord;
import com.example.fixedassets.model.AssetFullRecord;
import com.example.fixedassets.model.BatchInfo;
...
public class AcquisitionService {
    ...
    public int countBatchTransactions(int companyNo, int batchNo) {
        Integer n = jdbc.queryForObject(
            ...
            Integer.class, companyNo, batchNo);
        return n != null ? n : 0;
    }
    ...
    public String validateCodes(int companyNo, AcquisitionRecord rec) {
        ...
        if (!rec.attachTo.isBlank()) {
            Integer cnt = jdbc.queryForObject(
                ...
                Integer.class, companyNo, rec.attachTo);
            if (cnt == null || cnt == 0)
    ...
    public String checkDuplicate(int companyNo, String assetNo) {
        Integer n = jdbc.queryForObject(
            ...
            Integer.class, companyNo, assetNo);
        return (n != null && n > 0)
    ...
    public String checkAlreadyInBatch(int companyNo, int batchNo, String assetNo) {
        Integer n = jdbc.queryForObject(
            ...
            Integer.class, companyNo, batchNo, assetNo);
        return (n != null && n > 0)
    ...
    private void insertAsset(int companyNo, int batchNo,
        ...
        jdbc.update(
            ...
            ") VALUES (" +
            "?,?,?,"     +  //  1-3:  company_no, asset_no, alpha_code
            "?,?,?,?,"   +  //  4-7:  ak1
            "?,?,?,?,"   +  //  8-11: ak2
            "?,?,?,"     +  // 12-14: ak3
            "?,?,"       +  // 15-16: attach_to, attach
            "?,?,?,?,?,?,"+ // 17-22: desc1, desc2, loc, dept, grp, subgrp
            "?,?,"       +  // 23-24: status, status_ref
            "?,?,?,?,?," +  // 25-29: site, acqn_date, acqn_type, qty, int_order
            "?,?,?,"     +  // 30-32: supp_name, supp_no, supp_inv_no
            "?,?,?,?,"   +  // 33-36: leased, contract, paymt_amt, freq
            "?,?,?,"     +  // 37-39: lse_expiry, resid, tot_contr
            "?,?,?,?,"   +  // 40-43: curr_ins, repl_new, repl_as_at, ins_type
            "?,?,?,?,?,?,?"+ // 44-50: disposal, actual, tax_depn, book_depn, interest, write_down (extra ? for write_down_date)
            "?,?,?,"     +  // 51-53: tax_method, code, freq — note: disposal thru interest is 44-48, write_down 49, tax starts 50
            "?,?,?,?,"   +  // 54-57: tax_rate1, rate2, calc_ind, base
            "?,?,?,"     +  // 58-60: start_tax, accum_tax, accum_adj
            "?,?,"       +  // 61-62: last_tax_depn, last_adj
            "?,?,?,"     +  // 63-65: book_method, code, freq
            "?,?,?,?,"   +  // 66-69: book_rate1, rate2, calc_ind, base
            "?,?,?,"     +  // 70-72: start_bk, accum_bk, accum_adj
            "?,?,"       +  // 73-74: last_bk_depn, last_adj
            "?,?,?,"     +  // 75-77: last_reval_date, val, reserve
            "?,?,"       +  // 78-79: last_tax_reval_date, val
            "?,?,"       +  // 80-81: last_tfr, retmt_date
            "?,?,?,"     +  // 82-84: retmt_proceeds, bk_profit, tx_profit
            "?,?,?,?,"   +  // 85-88: parent_rate_orig, curr, maths, ind
            "?,?,?,"     +  // 89-91: parent_bk_cost, accum, reval
            "?,?,?,"     +  // 92-94: pool_flag, pool_bk_bal, pool_bk_date
            "?,?,?,"     +  // 95-97: pool_tax_bal, pool_tax_date, pool_acqn_flag
            "?,?,?,?,"   +  // 98-101: post_cl, post_ba, ledger_type, ledger_code
            "?,?,"       +  // 102-103: ba_ledger_id, ba_primary
            "?,"         +  // 104: note_no
            "?,?,?,?,?,?"   // 105-110: audit
            + ")",
    ...
    public ImportResult importAsset(int companyNo, int batchNo,
        ...
        Integer existsPosted = jdbc.queryForObject(
            "SELECT COUNT(*) FROM FAASSET " +
            "WHERE company_no=? AND asset_no=? AND asset_status<>'U'",
            Integer.class, companyNo, rec.assetNo);
        ...
        Integer inBatch = jdbc.queryForObject(
            "SELECT COUNT(*) FROM FATRXIN " +
            "WHERE company_no=? AND batch_no=? AND asset_no=?",
            Integer.class, companyNo, batchNo, rec.assetNo);
