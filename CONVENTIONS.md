...
- **`pagroup.paid_thru_to_X` is a Landmark Julian INT, NOT a DATE and NOT YYMMDD**. Epoch: **1899-12-31 = day 0**. e.g. `44773` = 2022-07-31, `43758` ≈ 2019-10-19, `50196` ≈ 2037-05-22. Helper: `landmarkDayToDate(int)` in `TimesheetEntryController`. Confirmed against live data 2026-05-16 (user-supplied pagroup.csv). NB: `MainMenuController.dayNoToDate` decodes a *different* format (YYMMDD-packed) used by other COBOL date fields — both conventions coexist in the data. Always check which you're dealing with.
